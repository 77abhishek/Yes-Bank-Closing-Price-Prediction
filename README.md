...
Regression Analysis On Yes Bank Stock Closing Price
...
**Project Goals:** The primary objectives of this capstone project are:
       ...
       a.)To preprocess and prepare the stock data for regression analysis.
       b.)To perform feature engineering to create relevant features for modeling.
       c.)To implement and evaluate different regression models for price prediction.
       d.)To select the best-performing model and provide predictions for Yes Bank's closing prices.
...
**4. Evaluation Model:** Evaluation will be based on various regression metrics to determine the model's accuracy in predicting Yes Bank's closing prices. Common metrics include Mean Absolute Error (MAE), Root Mean Square Error (RMSE), R-squared (R2) .
